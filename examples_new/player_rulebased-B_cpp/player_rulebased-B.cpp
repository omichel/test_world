...
class RuleBased : public aiwc::Participant {
  
  static constexpr double PI = 3.1415926535;

public:
  RuleBased(char **argv) : aiwc::Participant(argv) {}
  virtual ~RuleBased() {}
  ...
  auto get_coord(aiwc::game_frame f)
  {
    //Get data in the frame
    decltype(cur_posture) cur;
    decltype(cur_posture_op) cur_op;
    std::array<double, 2> pos_ball = { (f.coordinates).ball.x, (f.coordinates).ball.y };

    for(std::size_t id = 0; id < 5; ++id) {
      cur[id][X]  = (f.coordinates).robots[MYTEAM][id].x;
      cur[id][Y]  = (f.coordinates).robots[MYTEAM][id].y;
      cur[id][TH] = (f.coordinates).robots[MYTEAM][id].th;
      cur[id][ACTIVE] = (f.coordinates).robots[MYTEAM][id].active;
      cur[id][TOUCH] = (f.coordinates).robots[MYTEAM][id].touch;

      cur_op[id][X]  = (f.coordinates).robots[OPPONENT][id].x;
      cur_op[id][Y]  = (f.coordinates).robots[OPPONENT][id].y;
      cur_op[id][TH] = (f.coordinates).robots[OPPONENT][id].th;
      cur_op[id][ACTIVE] = (f.coordinates).robots[MYTEAM][id].active;
      cur_op[id][TOUCH] = (f.coordinates).robots[MYTEAM][id].touch;
    }
  ...
  void update(aiwc::game_frame frame) override {
    if (frame.reset_reason == aiwc::GAME_START) {
        ...
        std::tie(cur_posture, cur_posture_op, cur_ball) = get_coord(frame);
        return;
    }
    ...
    for(std::size_t id = 0; id < 5; ++id) {
      speeds.push_back(robot_wheels[id][0]); // left
      speeds.push_back(robot_wheels[id][1]); // right
    }

    // send wheel speed data to the simulator
    set_speeds(speeds);

    //std::cout << cur_ball[0] << std::endl;
    //std::cout << fwd_idx << std::endl;
  }
...
private: // member variable
  aiwc::game_frame previous_frame;
...
int main(int argc, char **argv) {
  RuleBased *player = new RuleBased(argv);
  player->run();
  delete player;
  return 0;
}